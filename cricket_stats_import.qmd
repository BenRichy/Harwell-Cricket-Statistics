...
for (i in 1:nrow(results_data)) {
    ...
    data_players <- data_players |> bind_rows(results_detail_players_home)
    data_batting <- data_batting |> bind_rows(results_detail_batting |> mutate(test = 2))
    data_partnerships <- data_partnerships |> bind_rows(results_detail_partnerships)
    data_bowling <- data_bowling |> bind_rows(results_detail_bowling)
}
