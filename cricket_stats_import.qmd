...
team_ids <- c(284082,15138,15137, 378560)
saturday_team_id <- c(284082, 378560)
...
FP_ball_by_ball_annual <- "app/data/ball-data-dump-year/"
FP_match_type_annual <- "app/data/match-date-to-league-year/"


play_cricket_get_json <- function(url, max_attempts = 3) {
  if (requireNamespace("chromote", quietly = TRUE)) {
    browser_text <- NULL
    browser_path <- chromote::chrome_versions_add("119.0.6034.3", "chrome")

    for (attempt in seq_len(max_attempts)) {
      browser_text <- tryCatch({
        suppressWarnings(
          chromote::with_chromote_chrome(browser_path, {
            browser_session <- get("ChromoteSession", asNamespace("chromote"))[['new']]()
            browser_session[['default_timeout']] <- 30

            browser_session[['go_to']](url)
            browser_session[['Runtime']][['evaluate']]("document.body.innerText")[['result']][['value']]
          })
        )
      }, error = function(e) {
        NULL
      ...
      if (!is.null(browser_text)) break
      Sys.sleep(2)
    }

    if (!is.null(browser_text)) {
      return(fromJSON(browser_text))
    }
  }

  response <- GET(url, user_agent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36"))
  response_text <- content(response, as = "text", encoding = "UTF-8")

  if (status_code(response) >= 300) {
    response_summary <- trimws(gsub("<[^>]+>", " ", substr(response_text, 1, 250)))
    stop(
      sprintf(
        "Play-Cricket API request failed with HTTP %s. %s",
  ...
  if (!grepl("json", http_type(response), ignore.case = TRUE)) {
    stop("Play-Cricket API returned a non-JSON response. Check the URL, token, and site access.")
  }
...
get_matches_query <- sprintf(
  "https://play-cricket.com/api/v2/result_summary.json?&site_id=%s&season=%s&api_token=%s",
    club_id,
    season[year],
    api_key
)

results_data_year <- play_cricket_get_json(get_matches_query)$result_summary
...
get_matches_summary_query <- sprintf(
  "https://play-cricket.com/api/v2/matches.json?&site_id=%s&season=%s&api_token=%s",
    club_id,
    season[year],
    api_key
)

matches_data <- play_cricket_get_json(get_matches_summary_query)$matches |> 
  select(match_id = id,
         ground_latitude,
         ground_longitude)
}
...
#start j as 0, use this to deal with rows being skipped
j <- 0
...
for (i in 1:nrow(results_data)) {
  ...
  # if the result was cancelled then go to the next match
    if (results_data["result"][[1]][[i]] %in% c("C")) {
        next
    }
  
  #if not Cancelled, then increment j
  j <- j + 1
  
  #set match id to the correct match
  innings_info[j,"match_id"] <- results_data[i,"id"]
  ...
  #batting variables
  innings_info[j,"batting_runs_scored"] <- as.integer(innings_data_frame_team[1,"runs"])
  ...
  innings_info[j,"batting_overs_faced_overs"] <- batting_overs_faced_calc_overs
  innings_info[j,"batting_overs_faced_balls"] <- batting_overs_faced_calc_balls
  
  innings_info[j,"batting_wickets_lost"] <- as.integer(innings_data_frame_team[1,"wickets"])
  
  #bowling variables
  innings_info[j,"bowling_runs_conceded"] <- as.integer(innings_data_frame_oppo[1,"runs"])
  #get the number of overs and balls as integers
  ...
  innings_info[j,"bowling_overs_bowled_overs"] <- bowling_overs_faced_calc_overs
  innings_info[j,"bowling_overs_bowled_balls"] <- bowling_overs_faced_calc_balls
  
  innings_info[j,"bowling_wickets_taken"] <- as.integer(innings_data_frame_oppo[1,"wickets"])
  innings_info[j,"bowling_wides_conceded"] <- as.integer(innings_data_frame_oppo[1,"extra_wides"])
  innings_info[j,"bowling_no_balls_conceded"] <- as.integer(innings_data_frame_oppo[1,"extra_no_balls"])
  innings_info[j,"bowling_byes_conceded"] <- as.integer(innings_data_frame_oppo[1,"extra_byes"])
  innings_info[j,"bowling_leg_byes_conceded"] <- as.integer(innings_data_frame_oppo[1,"extra_leg_byes"])
...
for (i in 1:nrow(results_data)) {
    ...
    get_matches_detail_query <- sprintf(
      "https://play-cricket.com/api/v2/match_detail.json?&match_id=%s&api_token=%s",
        match_id,
        api_key
    )
    ...
    # pull out the detail
    results_detail_data <- play_cricket_get_json(get_matches_detail_query)$match_details
...
Concatenate the ball-by-ball data and the summary of fixtures by league/team from annual data
```{r ConcatAnnualBallByBall}

#read in ball by ball data
ball_data_files <- list.files(FP_ball_by_ball_annual)
complete_ball_data <- data.frame()


for (file_name in 1:length(ball_data_files)) {
  ball_data_year <- read_csv(paste0(FP_ball_by_ball_annual,ball_data_files[file_name])) |> 
    mutate(`Start Time` = paste0(`Start Time`))
  
  #combine all ball by ball data
  complete_ball_data <- complete_ball_data |> 
    bind_rows(ball_data_year)
  
}

#output combined data
write_csv(complete_ball_data, "app/data/ball-data-dump.csv")


#repeat for match types
#read in ball by ball data
match_type_files <- list.files(FP_match_type_annual)
complete_match_type <- data.frame()


for (file_name in 1:length(match_type_files)) {
  match_type_year <- read_csv(paste0(FP_match_type_annual,match_type_files[file_name]))
  
  #combine all ball by ball data
  complete_match_type <- complete_match_type |> 
    bind_rows(match_type_year)
  
}

#output combined data
write_csv(complete_match_type, "app/data/match_date_to_league.csv")
